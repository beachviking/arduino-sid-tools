...
// A simplified version of Phil's original version from his excellent audio tools repo:
// https://github.com/pschatzmann/arduino-audio-tools
// This version uses std::vector instead of Phil's Vector implementation

// global reference to access from static callback methods
class ButtonActions;
static ButtonActions *selfButtonActions = nullptr;
...
public:
  ...
  /// Default constructor
  ButtonActions(bool useInterrupt = false) {
    selfButtonActions = this;
  ...
  void add(int pin, void (*actionOn)(bool pinStatus, int pin, void *ref),
    ...
    log_i("ActionLogic::add pin: %d / logic: %d", pin, activeLogicPar);
    if (pin >= 0) {
      // setup pin mode
...
protected:
  ...
  static void ButtonActionsISR() { selfButtonActions->processAllActions(); }
  ...
  void setupPin(int pin, ActiveLogic logic) {
    ...
    if (use_pin_mode) {
      if (logic == ActiveLow) {
        ...
        log_i("pin %d -> INPUT_PULLUP", pin);
      } else {
        pinMode(pin, INPUT);
        log_i("pin %d -> INPUT", pin);
      }
    }

#if !defined(IS_MIN_DESKTOP)
    if (use_pin_interrupt) {
      attachInterrupt(digitalPinToInterrupt(pin), ButtonActionsISR, CHANGE);
    }
#endif
  }
};
